code segment	'code'
	...
	translateFrom 				DB	'[WXIO'						; символы для замены
  ...
  replaceWith DB 'ХЦЧШЩ{WXIO'
	...
	charToCursiveIndex 			DB 'С'							; символ для замены
	...
	; формирование таблицы идет по строке большей длины (1я строка).
	signatureLine1				DB	179, 'Панов Герман Дмитриевич', 179
	Line1_length 					equ	$-signatureLine1
	signatureLine2				DB	179, 'ИУ5-42                 ', 179
	Line2_length 					equ	$-signatureLine2
	signatureLine3				DB	179, 'Вариант #13            ', 179
	...
	helpMsg DB '>tsr.com [/?]', 10, 13
			...
			DB '  F9  - включение и отключения курсивного вывода русского символа С', 10, 13
			DB '  F1  - включение и отключение частичной русификации клавиатуры([WXIO -> ХЦЧШЩ)', 10, 13
			DB '  F2  - включение и отключение режима замены символов (строчные -> прописные)', 10, 13
	...
	; блокируем
	_block:
		mov ES:[BX], AX ; блокировка ввода символа
		xor AX, AX
